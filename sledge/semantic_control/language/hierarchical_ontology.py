...
class HierarchicalScenePath:
    ...
    def nuplan_projection(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "language_actor_detail": self.attributes.get(
                "language_actor_detail", "unspecified"
            ),
            ...
            "warnings": (
                [] if compatible else ["actor_category_requires_downstream_projection"]
            ),
        }
...
ACTOR_TYPE_TO_INTERACTIONS: Dict[str, Set[str]] = {
    ...
    "debris": {
        "lane_blocking",
        "partial_lane_occupation",
        "sudden_obstacle_appearance",
    },
...
INTERACTION_TO_TRIGGERS: Dict[str, Set[str]] = {
    ...
    "intersection_crossing": {
        "actor_enters_ego_lane",
        "ego_reaches_conflict_area",
    },
...
def _clean(value: Any, default: str = "unknown") -> str:
    ...
    return (
        str(value).strip().lower().replace("-", "_").replace(" ", "_") or default
    )
...
class HierarchicalSceneResolver:
    ...
    def resolve(
        self, frame: EventFrame, spec: Mapping[str, Any]
    ) -> HierarchicalScenePath:
        slots = dict(spec.get("semantic_slots", {}) or {})
        sentence = frame.sentence or str(
            spec.get("evidence", {}).get("sentence", "")
        )

        road, road_source, road_conf = self._road_topology(frame, slots, sentence)
        traffic_space, traffic_source, traffic_conf = self._traffic_space(
            frame, slots, sentence, road
        )
        (
            actor_group,
            actor_type,
            ...
            actor_source,
            actor_conf,
        ) = self._actor(frame, slots, sentence, traffic_space)
        ...
        trigger, trigger_source, trigger_conf = self._trigger(
            frame, slots, sentence, interaction
        )
        response, response_source, response_conf = self._response(
            frame, slots, sentence, interaction
        )
        risk, risk_source, risk_conf = self._risk(
            frame, slots, sentence, interaction
        )
        ...
        path = HierarchicalScenePath(
            ...
            attributes={
                ...
                "absolute_direction_hint": self._absolute_direction_hint(
                    source_region
                ),
                "response_status": (
        ...
        for index, node_type in enumerate(NODE_ORDER):
            ...
            parent_value = (
                "ego_centered_hazard_scene"
                if index == 0
                else values[parent_type]
            )
            next_node_type = (
                NODE_ORDER[index + 1]
                if index + 1 < len(NODE_ORDER)
                else "executable_parameters"
            )
            allowed_at_level = tuple(
                sorted(
                    self.allowed_values(
                        node_type, parent_type, parent_value, values
                    )
                )
            )
            allowed_children = tuple(
                sorted(
                    self.allowed_values(
                        next_node_type, node_type, values[node_type], values
                    )
                )
            )
            ...
            path.nodes.append(
                HierarchyNode(
                    ...
                    evidence=self._node_evidence(
                        frame, node_type, values[node_type]
                    ),
    ...
    def allowed_values(
        ...
        if node_type == "primary_actor_group":
            return set(
                TRAFFIC_SPACE_TO_ACTOR_GROUPS.get(parent_value, set())
            )
        ...
        if node_type == "auxiliary_entity":
            return set(
                INTERACTION_TO_AUXILIARIES.get(parent_value, {"none"})
            )

        interaction = selected.get("hazard_interaction", "unknown")
        if node_type == "source_region":
            return set(
                INTERACTION_TO_SOURCE_REGIONS.get(
                    interaction, {"front_same_lane"}
                )
            )
        if node_type == "target_region":
            return set(
                INTERACTION_TO_TARGET_REGIONS.get(interaction, {"ego_path"})
            )
        ...
        if node_type == "motion_direction":
            return set(
                INTERACTION_TO_MOTION_DIRECTIONS.get(
                    interaction, {"toward_ego_path"}
                )
            )
        if node_type == "trigger_event":
            return set(
                INTERACTION_TO_TRIGGERS.get(
                    interaction, {"ego_reaches_conflict_area"}
                )
            )
        if node_type == "ego_required_response":
            return set(
                INTERACTION_TO_RESPONSES.get(interaction, {"brake"})
            )
    ...
    def allowed_children(
        ...
        return self.allowed_values(
            child_type, parent_type, parent_value, selected
        )
    ...
    def validate(self, path: HierarchicalScenePath) -> List[str]:
        ...
        for index, node in enumerate(path.nodes):
            ...
            if node.node_type != expected_type:
                issues.append(
                    f"hierarchy_order_mismatch:{index + 1}:"
                    f"{node.node_type}!={expected_type}"
                )
            if node.value not in set(node.allowed_values_at_level):
                issues.append(
                    f"invalid_value_at_level:{node.node_type}={node.value}"
                )
        ...
        if actor_type not in ACTOR_GROUP_TO_TYPES.get(
            values.get("primary_actor_group", ""), set()
        ):
            issues.append("actor_type_not_nuplan_projection_category")
        if interaction == "occluded_emergence":
            if visibility == "fully_visible":
                issues.append(
                    "occluded_emergence_requires_occluded_visibility"
                )
            if not auxiliary.endswith("_occluder"):
                issues.append("occluded_emergence_requires_occluder")
            if direction != "occluder_to_ego_path":
                issues.append(
                    "occluded_emergence_requires_unique_"
                    "occluder_to_ego_path_direction"
                )
        return issues

    @staticmethod
    def _road_topology(
        frame: EventFrame, slots: Mapping[str, Any], sentence: str
    ) -> Tuple[str, str, float]:
        raw = _clean(
            slots.get("road_topology", frame.road_context.road_type)
        )
        if raw == "intersection" or _contains(
            sentence, ["intersection", "junction"]
        ):
            return (
                "intersection",
                "explicit" if raw == "intersection" else "inferred",
                0.95,
            )
        if raw == "roundabout" or _contains(
            sentence, ["roundabout", "rotary", "traffic circle"]
        ):
            return (
                "roundabout",
                "explicit" if raw == "roundabout" else "inferred",
                0.97,
            )
        if raw in {"construction_zone", "work_zone"} or _contains(
            sentence, ["construction", "work zone", "roadwork"]
        ):
            return (
                "work_zone",
                "normalized" if raw != "unknown" else "inferred",
                0.94,
            )
        if raw in {"merge_diverge", "ramp_merge"} or _contains(
            sentence, ["ramp", "merge area", "lane drop", "diverge"]
        ):
            return "merge_diverge", "inferred", 0.82
        return (
            "straight_segment",
            "normalized" if raw != "unknown" else "hierarchical_default",
            0.9 if raw != "unknown" else 0.72,
        )
    ...
    def _traffic_space(
        ...
        layout = _clean(
            slots.get("road_layout", frame.road_context.lane_context)
        )
        source = _clean(
            slots.get("source_side", frame.main_event.source_relation)
        )
        if road == "intersection":
            if frame.ego_event.ego_maneuver == "left_turn" or _contains(
                sentence, ["left turn", "turns left"]
            ):
                return "left_turn_path", "explicit", 0.96
            if (
                motion == "lateral"
                and _clean(slots.get("actor_type")) == "vehicle"
            ):
                return "cross_traffic_zone", "inferred", 0.86
            return "straight_through", "hierarchical_default", 0.65
        if road == "roundabout":
            return (
                ("entry_path", "inferred", 0.95)
                if layout == "roundabout_entry"
                else ("circulating_lane", "hierarchical_default", 0.62)
            )
        if road == "merge_diverge":
            return "ramp_merge", "inferred", 0.82
        if road == "work_zone":
            return (
                ("closed_lane", "explicit", 0.92)
                if _contains(sentence, ["closed lane", "lane closed"])
                else ("partially_blocked_lane", "inferred", 0.8)
            )
        if _contains(sentence, ["crosswalk", "zebra crossing"]):
            return "crosswalk_zone", "explicit", 0.94
        if source in {"curbside", "from_curb"} or _contains(
            sentence,
            ["curb", "roadside", "sidewalk", "parked car", "parked truck"],
        ):
            return "curbside_zone", "inferred", 0.84
        if motion == "merging" or layout in {
            "adjacent_lane_cut_in",
            "multi_lane_road",
        }:
            return "same_direction_multi_lane", "inferred", 0.9
        if motion == "oncoming" or _contains(
            sentence, ["oncoming", "opposite lane", "opposing"]
        ):
    ...
    def _actor(
        ...
        if (
            actor_base == "pedestrian"
            or frame.main_actor.actor_class == "human_on_foot"
        ):
            if _contains(
                actor_text,
                ["child", "kid", "schoolkid", "boy", "girl"],
            ):
                detail = "child"
            elif _contains(
                actor_text, ["jogger", "runner", "running person"]
            ):
                detail = "jogger"
            elif _contains(actor_text, ["wheelchair"]):
                detail = "wheelchair_user"
            else:
                detail = "adult_or_unspecified"
            return (
                "vulnerable_road_user",
                "pedestrian",
                detail,
                "normalized_to_nuplan",
                0.99,
            )
        if actor_base == "cyclist" or frame.main_actor.actor_class == "cyclist":
            if _contains(
                actor_text, ["e-bike", "ebike", "electric bicycle"]
            ):
                detail = "ebike"
            elif _contains(actor_text, ["scooter"]):
                detail = "scooter"
            else:
                detail = "bicycle"
            return (
        ...
        if (
            actor_base in {"traffic_object", "static_obstacle"}
            or frame.main_actor.actor_class == "traffic_object"
        ):
            if _contains(actor_text, ["cone"]):
                ...
                actor_text, ["parked vehicle", "parked car"]
            ):
        ...
        if traffic_space == "cross_traffic_zone" or _contains(
            sentence, ["cross traffic", "from the side road"]
        ):
            return (
                "vehicle",
                "cross_traffic_vehicle",
        ...
        if traffic_space in {"entry_path", "ramp_merge"} or _contains(
            sentence, ["merging vehicle", "merge into"]
        ):
    ...
    def _interaction(
        ...
        motion = _clean(
            slots.get("motion_geometry", frame.main_event.motion_axis)
        )
        event = _clean(
            slots.get(
                "fine_grained_conflict_type", frame.main_event.event_type
            )
        )
        risk = _clean(slots.get("risk_level"), "moderate")
        occluded = bool(
            slots.get("occlusion_enabled", frame.occlusion.enabled)
        )
        allowed = ACTOR_TYPE_TO_INTERACTIONS.get(actor_type, set())

        if (
            event == "roundabout_entry_conflict"
            and "roundabout_entry_conflict" in allowed
        ):
            return "roundabout_entry_conflict", "inferred", 0.97
        if occluded and "occluded_emergence" in allowed:
            explicit = frame.occlusion.enabled or _contains(
                sentence,
                ["behind", "occluded", "emerges", "appears from"],
            )
            return (
                "occluded_emergence",
        ...
        if actor_type in {"pedestrian", "cyclist"}:
            candidate = (
                "enter_ego_lane"
                if event == "enter_ego_lane"
                else "path_crossing"
            )
            return (
                _first_allowed(candidate, allowed, "path_crossing"),
                "inferred",
                0.93,
            )
        if actor_type == "lead_vehicle":
            if event == "hard_stop_ahead" or _contains(
                sentence,
                [
                    "hard brake",
                    "brakes hard",
        ...
        if (
            actor_type
            in {"adjacent_vehicle", "merging_vehicle", "generic_vehicle"}
            and motion == "merging"
        ):
            candidate = (
                "aggressive_cut_in"
                ...
                or _contains(
                    sentence,
                    ["aggressive", "tight gap", "no room", "squeezes"],
                )
                else "cut_in"
            )
            return (
                _first_allowed(candidate, allowed, "lane_change"),
                "inferred",
                0.9,
            )
        if (
            actor_type in {"oncoming_vehicle", "generic_vehicle"}
            and motion == "oncoming"
        ):
            candidate = (
                "left_turn_across_oncoming"
                if frame.ego_event.ego_maneuver == "left_turn"
                or _contains(sentence, ["left turn", "turns left"])
                else "oncoming_path_conflict"
            )
            return (
                _first_allowed(
                    candidate, allowed, "oncoming_path_conflict"
                ),
                "inferred",
                0.94,
            )
        if actor_type == "cross_traffic_vehicle" or (
            motion == "lateral" and actor_type.endswith("vehicle")
        ):
            return (
                _first_allowed(
                    "intersection_crossing", allowed, "path_crossing"
                ),
                "inferred",
                0.84,
            )
        ...
        if (
            actor_type in ACTOR_GROUP_TO_TYPES["static_object"]
            or motion == "static"
        ):
            return (
                _first_allowed(
                    "lane_blocking", allowed, "partial_lane_occupation"
                ),
                "inferred",
                0.9,
            )
        return (
            (sorted(allowed)[0], "hierarchical_default", 0.4)
            if allowed
            else ("path_crossing", "hierarchical_default", 0.4)
        )
    ...
    def _auxiliary(
        ...
        if interaction == "occluded_emergence":
            raw = _clean(
                slots.get("occluder_type", frame.occlusion.occluder_type)
            )
            ...
            elif raw == "parked_vehicle" or _contains(
                sentence, ["parked car", "parked vehicle"]
            ):
            ...
            return (
                _first_allowed(
                    candidate, allowed, "generic_vehicle_occluder"
                ),
                (
                    "normalized_explicit"
                    if explicit
                    else "hierarchical_default"
                ),
                0.98 if explicit else 0.62,
            )
        if (
            _contains(sentence, ["crosswalk", "zebra crossing"])
            and "crosswalk" in allowed
        ):
            return "crosswalk", "explicit", 0.96
        if (
            _contains(sentence, ["traffic light", "signal"])
            and "traffic_light" in allowed
        ):
        ...
        if (
            _contains(sentence, ["barrier"])
            and "temporary_barrier" in allowed
        ):
            return "temporary_barrier", "explicit", 0.93
        return (
            _first_allowed("none", allowed, sorted(allowed)[0]),
            "hierarchical_default",
            0.7,
        )
    ...
    def _source_region(
        ...
        # Explicit left/right wording is more specific than a generic
        # curb/roadside relation produced by the lower-level EventFrame parser.
        # Resolve the lexical side first so phrases such as "left roadside",
        # "on the left" and "parked truck on the right" cannot collapse to
        # the broader ``curbside`` bucket.
        if _contains(
            sentence,
            [
                "from the left",
        ...
        if candidate in allowed:
            return (
                ...
                (
                    "explicit"
                    if relation != "unknown"
                    or candidate in {"left_side", "right_side"}
                    else "inferred"
                ),
                0.9,
            )
    ...
    def _target_region(
        ...
        raw = _clean(
            slots.get("target_path", frame.main_event.target_relation)
        )
        mapping = {
            "ego_lane": "ego_lane",
            "ego_path": "ego_path",
        ...
        # Preserve the distinction between the lane region and the ego
        # trajectory.  Raw sentence evidence wins over an over-normalized slot.
        # This is important for the occluded-pedestrian benchmark where
        # ``ego_path`` denotes the trajectory that the pedestrian must intersect.
        if "ego_path" in allowed and _contains(
            sentence,
            [
                "ego path",
                "ego's path",
            ...
            return "ego_path", "explicit", 0.98
        if "ego_lane" in allowed and _contains(
            sentence,
            [
                "ego lane",
                "ego's lane",
            ...
            return "ego_lane", "explicit", 0.98

        candidate = mapping.get(raw, raw)
        if candidate in allowed:
            return (
                candidate,
                "explicit" if raw != "unknown" else "inferred",
                0.92,
            )
        fallback = (
            "ego_lane" if "ego_lane" in allowed else sorted(allowed)[0]
        )
    ...
    def _anchor_region(
        ...
        raw = _clean(
            slots.get(
                "anchor_region",
                frame.main_event.event_location_relation,
            )
        )
        ...
        if (
            raw == "roundabout_entry"
            or interaction == "roundabout_entry_conflict"
        ):
        ...
        if _contains(
            sentence,
            ["just ahead", "near ego", "close ahead", "directly ahead"],
        ):
    ...
    def _visibility(
        ...
        if interaction == "occluded_emergence" or occluded:
            ...
            explicit = frame.occlusion.enabled or _contains(
                sentence, ["behind", "hidden", "occluded"]
            )
            return (
                "fully_occluded",
                "explicit" if explicit else "inferred",
                0.96 if explicit else 0.9,
            )
    ...
    def _motion_direction(
        ...
        raw = _clean(
            slots.get(
                "conflict_direction",
                frame.main_event.motion_direction,
            )
        )
    ...
    def _trigger(
        ...
        if "actor_enters_ego_lane" in allowed and _contains(
            sentence, ["enter", "into the lane", "steps into"]
        ):
    ...
    def _response(
        ...
        if _contains(
            sentence, ["emergency brake", "hard brake", "panic brake"]
        ):
            return (
                _first_allowed(
                    "brake_or_emergency_brake", allowed, "brake"
                ),
                "explicit",
                0.95,
            )
        if _contains(sentence, ["yield"]):
            return (
                _first_allowed("yield", allowed, "brake"),
                "explicit",
                0.94,
            )
        if _contains(sentence, ["steer", "swerve", "evade"]):
            return (
                _first_allowed("steer", allowed, "brake"),
                "explicit",
                0.94,
            )
        if "brake_or_emergency_brake" in allowed:
            return (
                "brake_or_emergency_brake",
                "kinematic_pending",
                0.68,
            )
        if "yield" in allowed and interaction in {
            "left_turn_across_oncoming",
            "roundabout_entry_conflict",
            "unprotected_crossing",
        }:
        ...
        return (
            ("brake", "hierarchical_default", 0.7)
            if "brake" in allowed
            else (sorted(allowed)[0], "hierarchical_default", 0.65)
        )
    ...
    def _risk(
        ...
        if _contains(
            sentence, ["critical", "imminent collision", "unavoidable"]
        ):
    ...
    @staticmethod
    def _node_evidence(
        frame: EventFrame, node_type: str, value: str
    ) -> str:
        sentence = frame.sentence
        if node_type in {"primary_actor_group", "primary_actor_type"}:
            return frame.main_actor.text or _span(
                sentence,
                ["child", "pedestrian", "person", "cyclist"],
            )
        if (
            node_type == "hazard_interaction"
            and value == "occluded_emergence"
        ):
            return _span(
                sentence,
                [
        ...
        if node_type == "visibility":
            return _span(
                sentence,
                ["from behind", "behind", "occluded", "hidden"],
            )
        if node_type == "risk_level":
            return _span(
                sentence,
        ...
        if (
            node_type == "motion_direction"
            and value == "occluder_to_ego_path"
        ):
            return "derived: selected occluder -> ego path"
        if (
            node_type == "ego_required_response"
            and value == "brake_or_emergency_brake"
        ):
            return (
                "derived: response pending sampled TTC and stopping distance"
            )
        return ""
